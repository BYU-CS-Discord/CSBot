...
export interface RoomsResponse {
	Rooms: [string, string][]; // [room_number, building_name]
}
...
export async function searchNow(building: string): Promise<RoomsResponse> {
	...
	const rooms = (result as RoomAvailabilityResult[]).map(r => [r.roomNumber, r.buildingName] as [string, string]);
...
export async function searchWhen(building: string, room: string): Promise<WhenResponse> {
	...
	const events = result as EventInfo[];
	...
	let busySince = dayEvents[0]!.start;
	...
	if (dayEvents.length > 1) {
		...
		for (let i = 0; i < dayEvents.length - 1; i++) {
			...
			const gapMinutes = (nextStartTime.getTime() - endTime.getTime()) / (1000 * 60);
	...
	const isInUse = mountainTime > busySince && mountainTime < busyUntil;
	...
	return {
		busySince: formatDate(busySince),
		busyUntil: formatDate(busyUntil),
		isInUse
	};
}
