...
export async function lookup(
...
): Promise<Array<RoomAvailabilityResult> | Array<EventInfo>> {
					...
					where: {
						AND: [
							...
							},
							{
								OR: [
									{
										AND: [{ startTime: { lte: timeA } }, { endTime: { gt: timeA } }],
									},
									{
										AND: [{ startTime: { lt: timeB } }, { endTime: { gte: timeB } }],
									},
									{
										AND: [{ startTime: { gte: timeA } }, { endTime: { lte: timeB } }],
									},
								],
							},
						],
	...
	if (timeType === 'when') {
		...
		return currentEvents.map((event: { name: string; startTime: string; endTime: string }) => ({
			name: event.name,
		...
		const conflictingRoomIds = conflictingEvents.map((event: { roomId: number }) => event.roomId);
		...
		const availableRooms = await db.rooms.findMany({
			where: {
				...
				description: 'CLASSROOM',
				...(building && building !== 'ANY' && buildingRecord
					? { buildingId: buildingRecord.id }
					: {}),
			},
			include: {
		...
		return availableRooms.map((r: { number: string; building: { name: string } }) => ({
			roomNumber: r.number,
