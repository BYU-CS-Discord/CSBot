 ...
 */
export function setupScraperCron(
	cronSchedule: string = '0 2 * * 0',
	yearTerm?: string
): CronJob {
			...
			try {
				const result = await scrapeRoomData(term, (progress) => {
					if (progress.buildings % 5 === 0 && progress.buildings > 0) {
						info(`[CRON] Progress: ${progress.buildings} buildings, ${progress.rooms} rooms, ${progress.events} events`);
					}
				});

				info(`[CRON] Scheduled scrape complete! Buildings: ${result.buildings}, Rooms: ${result.rooms}, Events: ${result.events}`);
			} catch (err) {
