...
import { scrapeRoomData, getCurrentYearTerm, isValidYearTerm, getScraperStatus } from '../roomFinder/scraper.js';
...
export const scrapeRooms: GlobalCommand = {
	...
	async execute({ replyPrivately, options }): Promise<void> {
		...
		if (status.isRunning) {
			...
			const logsText = status.recentLogs.length > 0
		...
		scrapeRoomData(yearTerm, (progress) => {
			if (progress.buildings % 5 === 0 && progress.buildings > 0) {
				info(`[SCRAPER] Progress: Buildings=${progress.buildings}, Rooms=${progress.rooms}, Events=${progress.events}, Current=${progress.currentBuilding}`);
			}
		}).then((result) => {
			info(`[SCRAPER] ✅ Complete! Buildings: ${result.buildings}, Rooms: ${result.rooms}, Events: ${result.events}`);
		}).catch((err: unknown) => {
